...
//#linxi begin set0more
    int interval_set_manager::compare_interval_with_zero(const interval &now, const scoped_anum &zero) {
        if (!now.m_upper_inf) {
            int sgn = m_am.compare(now.m_upper, zero);
            if (sgn < 0)
                return -1;
        ...
        if (!now.m_lower_inf) {
            int sgn = m_am.compare(now.m_lower, zero);
            if (sgn > 0)
                return 1;
...
//#linxi begin set0more
        ...
        if (linxi_set_0_more) {
            ...
            bool available = true;
            for (unsigned i = 0; i < num; ++i) {
                int sgn = compare_interval_with_zero(s->m_intervals[i], zero);
                if (sgn == 0) {
                    available = false;
                    break;
                }
