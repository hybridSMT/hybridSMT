...
struct var_lit_term {
    ...
    uint64_t lit_idx;
    rational coff;
    var_lit_term(var_exp &_ve, uint64_t _term_idx, int _lit_idx, rational _coff)
        : var_idx(_ve.var_index),
          exponent(_ve.exponent),
          term_idx(_term_idx),
          lit_idx(_lit_idx),
          coff(_coff) {};
    var_lit_term(){};
...
struct coff_term {
    ...
    rational coff;
    coff_term(){};
    coff_term(int _term_idx,rational _coff):term_idx(_term_idx),coff(_coff){};
};
...
//if is_nra_lit: \sum coff*term<=key
//else:_vars[delta]
struct lit {
    ...
    rational                    key;
    int                         lits_index;
    rational                    delta;              // the current value of left side
    bool                        is_equal = false;   // true means a-b-k==0, else a-b-k<=0
    bool                        is_nra_lit = false;
...
struct variable{
    ...
    std::string                 var_name;
    rational                    low_bound = rational(-max_int);
    rational                    upper_bound = rational(max_int);
    bool                        is_nra;
...
public:
    ...
    std::vector<int>            nra_var_vec;
    std::vector<int>            bool_var_vec;
    std::vector<rational>       term_coffs;//given a current var, the term_coff[t] means the term_coff of var in term t
    ...
    int                         nra_var_idx_with_most_lits;
    bool                        use_pbs=false;
    bool                        is_idl=true;//if it is the IDL mode
    uint64_t                    last_op_var;
    rational                    last_op_value;//the last value and last var, x +1, at least at next step, x -1 is forbidden
    bool                        has_high_coff=false;//if there exists x*x
    // solution
    std::vector<rational>       _solution;
    std::vector<rational>       _best_solutin;
    int                         best_found_cost;
    int                         best_found_this_restart;
    int                         no_improve_cnt_bool=0;
    int                         no_improve_cnt_nra=0;
    bool                        is_in_bool_search=false;
    int                         _best_found_hard_cost_this_bool;
    int                         _best_found_hard_cost_this_nra;
    // control
    uint64_t                    _step;
    ...
    std::vector<rational>       operation_change_value_vec;
    std::vector<int>            operation_var_idx_bool_vec;
    std::vector<rational>       low_bound_tight;
    std::vector<rational>       up_bound_tight;
    std::vector<rational>       low_bound_slack;
    std::vector<rational>       up_bound_slack;
    Array                       *vars_with_tight_bound;
    Array                       *vars_with_slack_bound;
    std::chrono::steady_clock::time_point start;
    double                      best_cost_time;
    double                      _cutoff;
    rational                    min_ration;     // go through all lit (except those x<0), min_ration should smaller than its coff
    const int                   BMS_cnt;
    const int                   random_walk_clauses;
    const int                   random_walk_idx;
    std::mt19937                mt;
    ...
    std::vector<rational>          _pre_value_1;// the 1st pre-set value of a var, if the var is in the form of (a==0 OR a==1)
    std::vector<rational>          _pre_value_2;
    std::vector<bool>              has_only_pre_value;
    ...
    std::vector<rational>       fa_coff;    // fa_coff[x]=c means x==c*fa[x]
    ...
    void                        merge(int var_idx_1,int var_idx_2,const rational &  coff_1,const rational &  coff_2);//var_1*coff_1=var_2*coff_2
    void                        update_lit_equal(int lit_idx);
    uint64_t                    transfer_name_to_reduced_var(std::string & name,bool is_nra);//after the resolution vars are inserted into _vars
    uint64_t                    transfer_name_to_resolution_var(std::string &name,bool is_nra);//bool var is inserted into _resolution_var when build lit
    uint64_t                    transfer_name_to_tmp_var(std::string &name);//nra var is first inserted into _tmp_var when build lit, then inserted into _resolution_var when reduce var(x-y->z)
    ...
    ls_solver(int random_seed,unsigned ls_time);
    void                        make_space();
    ...
    int                         pick_critical_move(rational &best_value);
    int                         pick_critical_move_bool();
    void                        critical_move(uint64_t var_idx,const rational & change_value);
    void                        move_update(uint64_t var_idx,const rational& change_value);
    void                        move_update(uint64_t var_idx);//dedicated for boolean var
    void                        invert_lit(lit &l);
    rational                    delta_lit(lit &l);
    rational                    coff_in_term(uint64_t var_idx,uint64_t term_idx,int exponent);
    void                        add_coff(uint64_t var_idx_curr, bool use_tabu, int lit_idx, int &operation_idx,const rational & coff_1);
    void                        add_coff(uint64_t var_idx_curr, bool use_tabu, int lit_idx, int &operation_idx,const rational & coff_1,const rational & coff_2,const rational & coff_0);
    double                      TimeElapsed();
    void                        clear_prev_data();
    __int128_t                  devide(__int128_t a, __int128_t b);
    void                        insert_operation(uint64_t var_idx,const rational & change_value,int &operation_idx,bool use_tabu);
    ...
    void                        set_bounds(uint64_t var_idx_curr, bool use_tabu, int lit_idx, int &operation_idx,const rational &  coff_1);
    void                        insert_from_bounds(bool use_tabu, int &operation_idx);
    void                        reset_bounds();
    void                        select_best_operation_from_vec(int operation_idx,int &best_score,int &best_var_idx,rational &best_value);
    ...
    void                        up_bool_vars();
    //calculate score
    int                         critical_score(uint64_t var_idx,const rational &  change_value);
    __int128_t                  critical_subscore(uint64_t var_idx,const rational &  change_value);
    //check
    bool                        check_solution();
    bool                        check_after_init();
    ...
    //handle ration
    inline rational mid_ration(rational & r_1,rational & r_2){return (r_1+r_2)/2;}
    //local search
    bool                        local_search();
};
}