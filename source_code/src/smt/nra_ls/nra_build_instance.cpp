...
bool cmp_ve(var_exp ve1,var_exp ve2){return (ve1.var_index<ve2.var_index)||(ve1.var_index==ve2.var_index&&ve1.exponent<ve2.exponent);}
...
void ls_solver::build_lits(std::string &in_string){
    ...
    if(vec.size()>2){
        ...
        if(vec.size()>6){
            ...
            for(;idx<vec.size();idx++){
                ...
                if(vec[idx]=="("){
                    ...
                    if((vec[idx][0]<'0'||vec[idx][0]>'9')&&(vec[idx][0]!='-')){//( * global_invc1_0 lam1n4 )
                        ...
                        while(denominator(coff)>denominator(min_ration)){min_ration/=rational(2);}
            ...
            if(vec[2]!="="||vec[3]=="(") {l->key=-rational(vec[++idx].c_str());}
            if(vec[2]==">="){invert_lit(*l);}
        }//( <= ( + x1 ( * -1 x2 ) x7 ( * -1 x8 ) ) 0 )
        else{
            l->lits_index=std::atoi(vec[0].c_str());
            if(vec[2]=="="&&vec.size()==6&&(vec[3][0]<'0'||vec[3][0]>'9')&&(vec[4][0]<'0'||vec[4][0]>'9')&&(vec[3][0]!='-')&&(vec[4][0]!='-')){
                ...
                if(vec[2]==">="){l->key=bound;l->coff_terms.push_back(coff_term((int)transfer_term_to_idx(new_t),rational(-1)));}
                else{l->key=-bound;l->coff_terms.push_back(coff_term((int)transfer_term_to_idx(new_t),rational(1)));}
                l->is_equal=(vec[2]=="=");
            }//( >= x 0 )
        }
...
void ls_solver::merge(int var_idx_1, int var_idx_2,const rational & coff_1,const rational & coff_2){//coff_1*var_1==coff_2*var_2
    int fa_1=find(var_idx_1),fa_2=find(var_idx_2);
    if(fa_1==fa_2){return;}
    rational fa_coff_1=fa_coff[var_idx_1]*coff_1, fa_coff_2=fa_coff[var_idx_2]*coff_2;//fa_coff_1*fa_1=fa_coff_2*fa_2
...
void ls_solver::update_lit_equal(int lit_idx){
    lit *l=&(_lits[lit_idx]);
    if(l->lits_index==0||!l->is_nra_lit){return;}
...
void ls_solver::build_instance(std::vector<std::vector<int> >& clause_vec){
        ...
        if(clause_vec[clause_idx].size()==1){
            ...
            if(l->coff_terms.size()==1&&is_single_var_term(_terms[l->coff_terms[0].term_idx])){
                ...
                if(clause_vec[clause_idx][0]>0){
                    ...
                    if(l->coff_terms[0].coff>0){new_low_bound=((-l->key)/(l->coff_terms[0].coff))+min_ration;}//ax+k>0  x>(-k)/a
                    else{new_upper_bound=((-l->key)/(l->coff_terms[0].coff))-min_ration;}//ax+k>1 x<(-k)/a
                }
    ...
    std::vector<bool> tautology_clauses_with_lits2(clause_vec.size(),false);//if the clause contains more than 2 lits, and one of them is nra_lit and true, then the clause is tautology
    for (int clause_idx=0;clause_idx<clause_vec.size();clause_idx++) {
        if(clause_vec[clause_idx].size()<2){continue;}
        for (auto l_idx : clause_vec[clause_idx]) {
            lit * l=&(_lits[std::abs(l_idx)]);
            if(l->lits_index==0&&l->is_nra_lit){tautology_clauses_with_lits2[clause_idx]=true;break;}
        }
    }
    reduce_vars();
    ...
    for (int clause_idx=0;clause_idx<clause_vec.size();clause_idx++) {
        ...
        for (auto l_idx : clause_vec[clause_idx]) {if(_lits[std::abs(l_idx)].lits_index==0){is_tautology=true;break;}}
        if(is_tautology||tautology_clauses_with_lits2[clause_idx]){continue;}
        for (auto l_idx : clause_vec[clause_idx]) {
            _clauses[_num_clauses].literals.push_back(l_idx);
            lit *l=&(_lits[std::abs(l_idx)]);
            if(l->lits_index==0){continue;}
            if(!l->is_nra_lit){_resolution_vars[l->delta.get_int32()].clause_idxs.push_back((int)_num_clauses);}
        }
        _num_clauses++;
    }
...
uint64_t ls_solver::transfer_name_to_reduced_var(std::string &name, bool is_nra){
    if(name2var.find(name)==name2var.end()){
        ...
        var.is_nra=is_nra;
        _vars.push_back(var);
        if(is_nra){nra_var_vec.push_back((int)_vars.size()-1);}
...
uint64_t ls_solver::transfer_name_to_resolution_var(std::string & name,bool is_nra){
    if(name2resolution_var.find(name)==name2resolution_var.end()){
        ...
        if(is_nra){nra_var_vec.push_back((int)_resolution_vars.size()-1);}
...
void ls_solver::reduce_vars(){
    ...
    if(use_pbs){_resolution_vars=_tmp_vars;}//if there is no boolean vars and all nra vars are in [0,1], then use pbs, and no need to reduce the vars
    else{
    name2var.clear();
    str2termidx.clear();
...
void ls_solver::unit_prop(){
    ...
    for(uint64_t clause_idx=0;clause_idx<_num_clauses;clause_idx++){//the unit clause is the undeleted clause containing only one bool var
        if(!_clauses[clause_idx].is_delete&&_clauses[clause_idx].literals.size()==1&&!_lits[std::abs(_clauses[clause_idx].literals[0])].is_nra_lit){unit_clause.push(clause_idx);}
    }
    while(!unit_clause.empty()){
        ...
        for(uint64_t cl_idx:_resolution_vars[unit_var].clause_idxs){
            ...
            for(uint64_t lit_idx=0;lit_idx<cl->literals.size();lit_idx++){
                ...
                if(!l->is_nra_lit&&l->delta==unit_var){//go through the clauses of the unit var, find the var in corresponding clause
                    if((is_pos_lit>0&&l_id_in_lits>0)||(is_pos_lit<0&&l_id_in_lits<0)){
                        ...
                        if(cl->literals.size()==1&&!_lits[std::abs(cl->literals[0])].is_nra_lit){unit_clause.push(cl_idx);}//if after deleting, it becomes unit clause
                    }
                    break;
                }
            }
    ...
    for(uint64_t bool_var_idx:bool_var_vec){
        ...
        for(int i=0;i<_resolution_vars[bool_var_idx].clause_idxs.size();i++){
            ...
            for(int l_var_sign:_clauses[clause_idx].literals){
                if(!_lits[std::abs(l_var_sign)].is_nra_lit&&_lits[std::abs(l_var_sign)].delta==(int)bool_var_idx){//make sure that it is a boolean literal and is exactly the one containing the var
        ...
        for(int i=0;i<pos_clause_size;i++){//pos clause X neg clause
            ...
            for(int j=0;j<neg_clause_size;j++){
                ...
                for(int k=0;k<_clauses[neg_clause_idx].literals.size();k++){
                    ...
                    if(_lits[std::abs(l_neg_lit)].delta!=(int)bool_var_idx||_lits[std::abs(l_neg_lit)].is_nra_lit){//the bool_var for resolution is not considered(that is \neg ( the lit is bool lit and it contains the var))
        ...
        for(int i=0;i<pos_clause_size;i++){//pos clause X neg clause
            ...
            for(int j=0;j<neg_clause_size;j++){
                ...
                for(int k=0;k<pos_lit_size;k++){
                    ...
                    if(_lits[std::abs(l_sign_idx)].is_nra_lit||_lits[std::abs(l_sign_idx)].delta!=(int)bool_var_idx){new_clause.literals.push_back(l_sign_idx);}
                }//add the lits in pos clause to new clause
                for(int k=0;k<neg_lit_size;k++){
                    int l_sign_idx=_clauses[neg_clause_idx].literals[k];
                    if(_lits[std::abs(l_sign_idx)].is_nra_lit||_lits[std::abs(l_sign_idx)].delta!=(int)bool_var_idx){
...
void ls_solver::reduce_clause(){
    ...
    _num_nra_lits=0;_num_bool_lits=0;
    ...
    for(variable &v:_resolution_vars){
        if(v.is_nra&&v.upper_bound!=max_int&&v.low_bound!=-max_int){transfer_name_to_reduced_var(v.var_name, true);}
        ...
        for(int k=0;k<_clauses[clause_idx].literals.size();k++){
            ...
            if(l->is_nra_lit){
                ...
                if(!lit_appear[std::abs(l_sign_idx)]){
                    ...
                    _vars[l->delta.get_int32()].literal_idxs.push_back(std::abs(l_sign_idx));//for a boolean var, its first lit_idx is the lit containing the var
                }
                _vars[l->delta.get_int32()].clause_idxs.push_back(clause_idx);
                _vars[l->delta.get_int32()].bool_var_in_pos_clause.push_back(l_sign_idx>0);//for a boolean var, if it is neg in a clause, it is false
    ...
    for(uint64_t term_idx=0;term_idx<_terms.size();term_idx++){
        ...
        for(var_exp &ve:t->var_epxs){
            ...
            if(ve.exponent>=2){has_high_coff=true;}
