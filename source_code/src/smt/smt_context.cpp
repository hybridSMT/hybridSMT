...
namespace smt {
    ...
    context::context(ast_manager & m, smt_params & p, params_ref const & _p):
        ...
        m_nra_ls_solver = new nra::ls_solver((int)m_fparams.m_random_seed, m_fparams.m_ls_time);
   ...
   static bool first_into_cxt=true;//only in 2nd round will the LS be called
        ...
        if (!theory_assumptions.empty()) {
            ...
            if(is_NIA){
                ...
                // else{
                    ...
                    print_clauses_vec(core_clauses);
                    if(m_nia_ls_solver->has_unidentified_lits){return check_finalize(l_undef);}
    ...
    lbool context::search() {
        ...
        while (true) {
            ...
            if(is_NIA&&ls_sat){return l_true;}
    ...
    void context::call_ls(const std::vector<std::vector<int> >& cl_vec, uint64_t ls_max_step,bool complete_ls,int &ls_best_found_cost,bool record_frequency){
        ...
        if(nia_ls_solver->local_search()){
            ls_sat=true;
            m_model_generator->reset();
            m_proto_model = m_model_generator->mk_model_ls(nia_ls_solver);
        }
