...
namespace smt {
    ...
    */
    ...
    void model_generator::mk_bool_model(lia::ls_solver *solver){
        unsigned sz = m_context->get_num_b_internalized();
        for (unsigned i = 0; i < sz; i++) {
            expr * p = m_context->get_b_internalized(i);
            if (is_uninterp_const(p)) {
    ...
    void model_generator::mk_bool_model(nra::ls_solver *solver){
        unsigned sz = m_context->get_num_b_internalized();
        for (unsigned i = 0; i < sz; i++) {
            expr * p = m_context->get_b_internalized(i);
            if (is_uninterp_const(p)) {
...
#define Black 2
    ...
    */
    ...
    void model_generator::mk_values_ls(nia::ls_solver *solver) {
        ...
        for (enode * n : m_context->enodes()) {
            ...
            if (is_uninterp_const(n->get_expr())) {
                func_decl * d = n->get_expr()->get_decl();
                if (m_hidden_ufs.contains(d)) continue;
                val=get_value_ls(a_fac,solver,n);
                register_value(val);
                m_asts.push_back(val);
                m_root2value.insert(n, val);
            }
        ...
        for (enode * n : m_context->enodes()) {
            if (is_uninterp_const(n->get_expr())) {
                func_decl * d = n->get_expr()->get_decl();
                if (m_hidden_ufs.contains(d)) continue;
                expr * val    = get_value(n,true);
                m_model->register_decl(d, val);
            }
        }
    }

    app * model_generator::get_value_ls(arith_factory &a_fac,nia::ls_solver *solver,enode * n){
        rational num;
        std::stringstream var_name_stream;
        ...
        return a_fac.mk_num_value(num,true);//只要在此处将val赋值便可以了
    }

    void model_generator::mk_values_ls(lia::ls_solver *solver) {
        arith_factory a_fac=arith_factory(m);
        rational num;
        for (enode * n : m_context->enodes()) {
            app *val;
    ...
    void model_generator::mk_values_ls(nra::ls_solver *solver) {
        arith_factory a_fac=arith_factory(m);
        rational num;
        for (enode * n : m_context->enodes()) {
            app *val;
            if (is_uninterp_const(n->get_expr())) {
                ...
                solver->print_var_solution(var_name,var_value);
                num=rational(var_value.c_str()).get_rational();
                val=a_fac.mk_num_value(num,false);//只要在此处将val赋值便可以了
                register_value(val);
                m_asts.push_back(val);
                m_root2value.insert(n, val);
            }
    ...
    app * model_generator::get_value(enode * n,bool is_ls) const {
        if(is_ls){return m_root2value[n];}
        else{return m_root2value[n->get_root()];}
    }
    ...
    */
    ...
    /**
       ...
       The "else" is missing.
    */
    void model_generator::mk_func_interps(bool is_ls) {
        ...
        for (unsigned i = 0; i < sz; i++) {
            ...
            if (num_args == 0 && include_func_interp(f)) {
                m_model->register_decl(f, get_value(n,is_ls));
            }
            else if (num_args > 0 && n->get_cg() == n && include_func_interp(f)) {
                ptr_buffer<expr> args;
                expr * result = get_value(n,is_ls);
                SASSERT(result);
                for (unsigned j = 0; j < num_args; j++) {
                    app * arg = get_value(n->get_arg(j),is_ls);
                    SASSERT(arg);
                    args.push_back(arg);
                }
    ...
    proto_model * model_generator::mk_model_ls(nia::ls_solver * solver) {
        ...
        mk_func_interps(true);
        finalize_theory_models();
        register_macros();       
        return m_model.get();
    }
    proto_model * model_generator::mk_model_ls(nra::ls_solver * solver) {
        init_model();
        register_existing_model_values();//注册已经存在的model 值，此处不会调用register_value 函数
        mk_bool_model(solver);
        mk_values_ls(solver);//此处会调用register_value函数
    ...
    proto_model * model_generator::mk_model_ls(lia::ls_solver * solver) {
        init_model();
        register_existing_model_values();//注册已经存在的model 值，此处不会调用register_value 函数
        mk_bool_model(solver);
        mk_values_ls(solver);//此处会调用register_value函数
